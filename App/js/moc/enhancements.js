...
function PowerCalc() {
  ...
  let powerTotal = parseFloat(
    txtPowerTotalValue.substring(0, txtPowerTotalValue.length)
  ...
  if (document.getElementById("enhanced_attr").checked) {
    ...
    if (!isNaN(value)) {
      if (unit_type === "structure" && value % 2 === 1) {
        ...
        uInchesInput.value = value - totalEnhancements;
...
$(document).ready(function () {
  ...
  $("#deflectionCheck").change(function () {
    ...
    // added this condition, if u_inches is 0 then stop the change and uncheck the box
    if (this.checked && !$(this).data("checked") && u_inches === 0) {
      alert("You cannot check it if the available Unit Inches (Ü) is 0.");
  ...
  function updateFields() {
    ...
    $("#txtAmorTotal").val(
      amorUpgradeVal <= 0 ? "1d6" : parseInt(amorUpgradeVal) + "d10"
    );
    ...
    switch (actionUpgradeVal) {
      case 0:
        ...
        break;
      case 1:
        $("#txtActionTotal").val("1d8");
        break;
      case 2:
        $("#txtActionTotal").val("1d10");
        break;
      case 3:
        $("#txtActionTotal").val("1d12");
        break;
    }
  ...
  upgradeInputs.forEach(function (elem) {
    $(elem).on("input", function () {
      ...
      if (diff > u_inches) {
        alert(
          "You cannot enter a number bigger than the available Unit Inches (Ü). Resetting to maximum available value."
        );
      ...
      totalEnhancements += diff; // Keep track of the enhancements made
  ...
  $(
    "#armor_impairmentCheck, #power_impairmentCheck, #speed_impairmentCheck, #action_impairmentCheck"
  ).change(function () {
    let valueImpairment = calculateValueImpairment();
  ...
  $("#action_impairmentCheck").change(function () {
    if (this.checked) {
      ...
      $("#halfmind").prop("checked", true).change();
      $("#extra_minds").val("0").attr("disabled", true).change();
    } else {
      ...
      $("#halfmind").prop("checked", false).change();
      $("#extra_minds").val("0").attr("disabled", false).change();
    }
  });
  ...
  $("#value_impairmentCheck").on("input", function () {
    ...
    let uinches = parseInt($("#u_inches").val());
    let new_uinches =
      valueImpairment > previousValueImpairment
